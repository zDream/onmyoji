...
//                robot.delay(1000 * 20);
//                String url = String.format("C:\\Users\\EDY\\Desktop\\test\\%s.png",i);
//                captureScreen(url);
//                String result = result(url);
//
//                Response response = JSON.parseObject(result, Response.class);
//                List<Lines> lines = response.getResult().getLines();
//                Optional<Lines> challenge = lines.stream().filter(k -> k.getText().equals("挑战")).findFirst();
//
//                if (challenge.isPresent()){
//
//                }

                System.out.println("移动鼠标到");
                //测 4 119 36 134 0
                //御灵副本
//                robot.mouseMove(1076, 536);
                robot.mouseMove(1096, 576);
//                robot.delay(10000);
                //按下鼠标左键
    ...
    public static String result(String filePath) {
        // 通用文字识别
        ...
        String result = "";
        try {
            byte[] imgData = readfile(filePath); // image
